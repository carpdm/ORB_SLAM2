...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    // mLeftDetector->detectAndCompute(im, cv::Mat(), mvKeys, mDescriptors);
    else
        (*mpORBextractorRight)(im, cv::Mat(), mvKeysRight, mDescriptorsRight);
    // mRightDetector->detectAndCompute(im, cv::Mat(), mvKeysRight,mDescriptorsRight);
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const float u = Config::fx() * PcX * invz + Config::cx();
    const float v = Config::fy() * PcY * invz + Config::cy();
    ...
    pMP->mTrackProjX = u;
    pMP->mTrackProjXR = u - Config::bf() * invz;
    pMP->mTrackProjY = v;
...
void Frame::ComputeBoW()
{
    if (mBowVec.empty()) {
        ...
        SingleInstance::GetVoc()->transform(vCurrentDesc, mBowVec, mFeatVec, 4);
    }
}

void Frame::UndistortKeyPoints()
{
    if (Config::DistCoef.at<float>(0) == 0.0) {
        mvKeysUn = mvKeys;
        return;
    }
    ...
    // Undistort points
    mat = mat.reshape(2);
    cv::undistortPoints(mat, mat, Config::K, Config::DistCoef, cv::Mat(), Config::K);
    mat = mat.reshape(1);
...
void Frame::ComputeStereoMatches()
{
    ...
    const float minZ = Config::b();
    const float minD = 0;
    const float maxD = Config::bf() / minZ;
    ...
    for (int iL = 0; iL < N; iL++) {
        ...
        if (bestDist < thOrbDist) {
            ...
            if (disparity >= minD && disparity < maxD) {
                ...
                mvDepth[iL] = Config::bf() / disparity;
                mvuRight[iL] = bestuR;
...
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    if (z > 0) {
        ...
        const float x = (u - Config::cx()) * z * Config::invfx();
        const float y = (v - Config::cy()) * z * Config::invfy();

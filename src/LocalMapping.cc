...
void LocalMapping::CreateNewMapPoints()
{
    ...
    const float &fx1 = Config::fx();
    const float &fy1 = Config::fy();
    const float &cx1 = Config::cx();
    const float &cy1 = Config::cy();
    const float &invfx1 = Config::invfx();
    const float &invfy1 = Config::invfy();
    ...
    for (size_t i = 0; i < vpNeighKFs.size(); i++) {
        ...
        const float &fx2 = Config::fx();
        const float &fy2 = Config::fy();
        const float &cx2 = Config::cx();
        const float &cy2 = Config::cy();
        const float &invfx2 = Config::invfx();
        const float &invfy2 = Config::invfy();
        ...
        for (int ikp = 0; ikp < nmatches; ikp++) {
            ...
            if (bStereo1)
                cosParallaxStereo1 = cos(2 * atan2(Config::b() / 2, mpCurrentKeyFrame->mvDepth[idx1]));
            else if (bStereo2)
                cosParallaxStereo2 = cos(2 * atan2(Config::b() / 2, pKF2->mvDepth[idx2]));
...
cv::Mat LocalMapping::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2)
{
    ...
    const cv::Mat &K1 = Config::K;
    const cv::Mat &K2 = Config::K;
        ...
        for (size_t i = 0, iend = vpMapPoints.size(); i < iend; i++) {
            ...
            if (pMP) {
                if (!pMP->isBad()) {
                    ...
                    if (pKF->mvDepth[i] > Config::ThDepth() || pKF->mvDepth[i] < 0)
                        continue;
